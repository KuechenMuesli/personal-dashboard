...
<div class="min-h-screen flex items-center justify-center p-4 font-sans text-white relative" style="background-color: var(--theme-body-bg, #121212);">
  ...
  <div class="w-full max-w-[380px] bg-neutral-800/80 backdrop-blur-xl border border-black/40 rounded-[24px] shadow-2xl p-8 relative z-10">
    ...
    <form method="POST" action={isSignUp ? "?/signup" : "?/login"} use:enhance={() => { loading = true; return async ({ update }) => { loading = false; update(); } }} class="space-y-4">
      ...
      <div>
        <label for="password" class="block text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-1.5 ml-1">Password</label>
    ...
    <div class="mt-8 flex items-center justify-between text-xs text-neutral-500">
      ...
      <span class="px-3 uppercase font-black tracking-widest text-[9px] text-neutral-600 whitespace-nowrap">or continue with</span>
      <hr class="w-full border-black/40" />
    </div>

    <div class="mt-6 flex gap-3">
      <button type="button" class="flex-1 flex items-center justify-center bg-black/20 border border-black/20 hover:bg-black/40 py-3 rounded-xl transition-all active:scale-[0.98]" onclick={() => signInWithOAuth('google')} title="Google">
      ...
      <button type="button" class="flex-1 flex items-center justify-center bg-black/20 border border-black/20 hover:bg-black/40 py-3 rounded-xl transition-all active:scale-[0.98]" onclick={() => signInWithOAuth('github')} title="GitHub">
